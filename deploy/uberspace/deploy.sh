...
echo "== sync frontend -> ~/html =="
rsync -az --delete --exclude '.htaccess' dist/ "$UBERSPACE":html/
...
ssh "$UBERSPACE" '
  ...
  supervisorctl restart fairteiler-api
  sleep 3 && supervisorctl status fairteiler-api
'
